...
void loop() {
  ...
  if (current_time - prev_time[2] > 20 && 1) {  //////////////////////////// LL EE DD
    ...
    if (rec_mode == 1) {
      ...
      set_led(0, 0, .33, .1 + (rp_led / 2));
    }


    set_led(7, .2, .5, float(arp_enable) * .2);
    set_led(9, .2, .5, float(hold_en) * .2);
    ...
    if (fmf > 0) {
      leds.setPixel(4, 0, 0, ledenv);
    }
    if (fmf < 0) {
      leds.setPixel(4, 0, 0, (ledenv * -1.0) + (max_brightness * 1.5));
    }
    ...
    if (filter_mod_final > 0) {
      leds.setPixel(6, 0, ledenv, 0);
    }
    if (filter_mod_final < 0) {
      leds.setPixel(6, 0, (ledenv * -1.0) + (max_brightness), 0);
    }
